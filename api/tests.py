...
class ArticleTests(APITestCase):
    ...
    def test_create_article_no_user(self):
        count=Article.objects.count()
        response=self.client.post(reverse('article-list'),{'title':'good','body':'ggodd'})
        self.assertNotEqual(response.status_code,201)
        self.assertEqual(Article.objects.count(),count)
    def test_edit_article(self):
        logged=self.client.login(username='menbawy',password='men@1234')
        self.assertTrue(logged)
        response=self.client.post(reverse('article-update',kwargs={'pk':12}),
                                                {'title':'good22','body':'ggood'})
        self.assertEqual(Article.objects.get(id=12).title,'good22')
    def test_edit_article_notowner(self):
        logged=self.client.login(username='menbawy',password='men@1234')
        self.assertTrue(logged)
        response=self.client.post(reverse('article-update',kwargs={'pk':14}),
                                                {'title':'good22','body':'ggood'})
...
class UserTests(APITestCase):
    ...
    def test_create_user(self):
        ...
        self.assertTrue(logged)
        response=self.client.post(reverse('user-list'),{'username':'apitester','email':'api@appp.com','password':'api@1234'})
        self.assertEqual(response.status_code,201)
